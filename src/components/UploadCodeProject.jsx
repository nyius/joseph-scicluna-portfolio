...
function UploadCodeProject() {
	...
	return (
		<form action="" className="flex flex-col bg-secondary h-fit w-1/4 p-5 border-4 border-primary-content" onSubmit={e => onSubmit(e)}>
			<label className="text-xl mt-2 font-bold">Coding Project Name</label>
			...
			<input id="link" type="text" defaultValue={link} placeholder="Enter link to project" className="input input-primary border-4" onChange={onMutate} required />
			<label className="text-xl mt-2">Github Link</label>
			<input id="github" type="text" defaultValue={github} placeholder="Enter link to project on github" className="input input-primary border-4" onChange={onMutate} required />
			...
			<input className="" type="file" id="image" max="1" accept=".jpg,.png,.jpeg" required onChange={onMutate} />
			<button className="btn btn-accent btn-large mt-5">Submit</button>
		</form>
	);
}

export default UploadCodeProject;
